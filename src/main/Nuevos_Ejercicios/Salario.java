...
public class Salario {
    public static double calc_bruto(int horas, double precio){
        double bruto;
        if (horas <= 35) {
            bruto = horas * precio;
        }else {
    ...
    public static double calc_impuestos(double bruto){
        double impuestos;
        if (bruto <= 500) {
            impuestos = 0;
        }else if (bruto <= 900){
        ...
        //Calcular bruto
        bruto = calc_bruto(horas, precio);

        //Calcular impuestos
        impuestos = calc_impuestos(bruto);
